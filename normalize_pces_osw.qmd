...
The purpose of this .qmd is to normalize the personal consumption expenditures multipliers for offshore wind. When inputting the multipliers into JEDI, just like all of the other multipliers, they are meant to add up to one. For some reason, only for this column, IMPLAN has the multipliers "un-normalized", so they are in their full form. In order to get them in the correct format, we need to normalize them to all add up to 1. 
